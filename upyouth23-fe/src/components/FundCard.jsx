...
const FundCard = ({ owner, category, title, description, target, deadline, amountCollected, image, handleClick }) => {
  ...
  return (
    <div className="sm:w-[288px] w-full rounded-[15px] bg-white cursor-pointer shadow-md" onClick={handleClick}>
      ...
      <div className="flex flex-col p-4">
        ...
        <div className="flex justify-between flex-wrap mt-[15px] gap-2">
          <div className="flex flex-col">
            <h4 className="font-epilogue font-semibold text-[14px] leading-[22px]">{amountCollected}</h4>
            <p className="mt-[3px] font-epilogue font-normal text-[12px] leading-[18px] sm:max-w-[120px] truncate">Raised of {target}</p>
          </div>
          <div className="flex flex-col">
        ...
        <div className="flex items-center mt-[20px] gap-[12px]">
          ...
          <p className="flex-1 font-epilogue font-normal text-[12px] truncate">by <span className="">{owner}</span></p>
        </div>
      </div>
    </div>
  )
